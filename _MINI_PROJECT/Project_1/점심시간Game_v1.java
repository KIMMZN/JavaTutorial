...
import java.util.Scanner;
	...
	public class 점심시간Game_v1 {
		...
		public static void main(String[] args) {
			//기본 array, 난이도 하; 
			//난이도 상 선택 후  game over시, 다시 돌아오면 난이도 
			//하에 난이도 상 array가 덮어 씌워져 있어
			//난이도 '하'를 선택해도 '상'배열의 단어들이 나와서
			//temp array를 만들어서 문제를 해겼했음.
			...
			//do while 반복문은 무조건 한번 이상 실행 됨.
			//while 조건식이 true라면 false가 될때까지 계속해서 do안의 코드를 반복실행.
			while(true) {
			...
			// c의 값이 "상"일 경우 밑에 for문에서의 a배열을 aA배열로 치환하여 진행하도록 하는 if문
			if (c.equals("상")) {
			...
			// c의 값이 "하"일 경우 밑에 for문에서의 a배열을 tempa 배열로 치환하여 진행하도록 하는 if문
			if (c.equals("하")) {
				for (int i=0; i<a.length; i++) {
					a[i] = tempa[i];
				}
